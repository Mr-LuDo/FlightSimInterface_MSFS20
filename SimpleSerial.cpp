	...
	if (syntaxfile_in.is_open()) {
		...
		while (syntaxfile_in) {			
			...
			if (syntax_name_ == syntax_type) {
				//cout << "syntax_name_ = " << syntax_name_ << endl;
				found = true;
				break;
			}
		}
...
string SimpleSerial::ReadSerialPort(int reply_wait_time, string syntax_type) {
	...
	while ((time(nullptr) - start_time) < reply_wait_time) {
		...
		//if (status_.fEof | status_.fTxim) {
		//	cout << "status_.fEof == true" << endl;
		//	return "status_.fEof == true.\n";;
		//}
		//if (status_.fCtsHold | status_.fDsrHold | status_.fRlsdHold | status_.fXoffHold | status_.fXoffSent) {
		//	cout << "Others" << endl;
		//	return "Others \n";
		//}
